...
var multi_scatter_tex := load("res://atmosphere/multi-scatter-lut.tres")

func _init():
	RenderingServer.call_on_render_thread(_initialize_compute_code)
#	# Can leave this commented if the sun is updating on transform changed
	#RenderingServer.connect("frame_pre_draw", update_lut)
...
func _create_uniform_set_for_sampling() -> RID:
	...
	uniform.binding = 1
	uniform.add_id(sampler)
	uniform.add_id(multi_scatter_tex.texture_rd)
	uniforms.push_back(uniform)
...
func _initialize_compute_code():
	...
	# Create our shader
	var shader_file = load("res://atmosphere/sky-lut.glsl")
...
func render_lut():
	...
	if sun:
		light_direction = (sun.transform.basis * Vector3(0.0, 0.0, 1.0)).normalized()
